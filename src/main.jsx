...
function Footer() {
  return <footer>
    <div className="footer-grid">
      ...
      <div><span>CONTACT</span><a href={`mailto:${company.email}`}>{company.email}</a><p>Bengaluru, India</p></div>
    </div>
...
function Contact() {
  ...
  return <><PageHero kicker="RESULT MINT / CONTACT" title={<>Let’s make<br/><em>something move.</em></>} intro="Tell us what you’re trying to achieve. We’ll start with the business problem, not a media plan."/><section className="section-shell contact-page"><div className="contact-grid"><Reveal><div className="contact-info"><div className="section-label">GET IN TOUCH</div><a className="email" href={`mailto:${company.email}`}>{company.email} ↗</a><div className="address"><small>VISIT US</small><p>{company.address}</p></div></div></Reveal><Reveal><form onSubmit={e=>{e.preventDefault();setSent(true);}}><label>Name *<input required name="name" placeholder="Your name"/></label><label>Work email *<input required type="email" name="email" placeholder="you@company.com"/></label><label>Company *<input required name="company" placeholder="Company name"/></label><label>Website<input name="website" placeholder="https://"/></label><label>What can we help with? *<select required name="service" defaultValue=""><option value="" disabled>Select a service</option>{services.map(([x])=><option key={x}>{x}</option>)}</select></label><label>Tell us about your goals *<textarea required name="message" rows="6" placeholder="What are you trying to achieve?"></textarea></label><button className="btn lime" type="submit">{sent ? 'Enquiry saved ↗' : 'Send enquiry ↗'}</button><small className="form-note">The form is ready for a Formspree or backend connection later without redesigning this page.</small></form></Reveal></div></section></>;
}
